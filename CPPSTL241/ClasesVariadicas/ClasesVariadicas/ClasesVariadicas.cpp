...
class MiPlantilla {
public:
	MiPlantilla(Args... args) : args(args...) {}

	void imprimir_argumentos() {
		imprimir_argumentos_recursivo(std::make_index_sequence<sizeof...(Args)>{}, args);
	}

private:
	template <std::size_t... Is>
	void imprimir_argumentos_recursivo(std::index_sequence<Is...>, const std::tuple<Args...>& tupla_args) {
		((std::cout << std::get<Is>(tupla_args) << std::endl), ...);
	}
...
int main() {
	MiPlantilla<int, double, std::string> objeto1(10, 3.14, "Hola");
	objeto1.imprimir_argumentos();

	MiPlantilla<float, int> objeto2(2.5, 7);
	objeto2.imprimir_argumentos();
...
#include <iostream>
#include <tuple>  // Necesario para utilizar std::tuple y std::get
...
private:
	// Función recursiva que imprime los argumentos de la tupla "args"
	template <std::size_t... Is>
	void imprimir_argumentos_recursivo(std::index_sequence<Is...>, const std::tuple<Args...>& tupla_args) {
		// Utilizamos el operador de expansión de la tupla para imprimir los argumentos uno por uno
		((std::cout << std::get<Is>(tupla_args) << std::endl), ...);
	}
	// La tupla "args" almacena los argumentos de plantilla que se le pasaron al constructor
	std::tuple<Args...> args;
public:
	// Constructor que acepta un número variable de argumentos de plantilla
	MiPlantilla(Args... args) : args(args...) {}

	// Función que imprime los argumentos de plantilla almacenados en la tupla "args"
	void imprimir_argumentos() {
		// Llamamos a la función "imprimir_argumentos_recursivo", pasando un índice de secuencia y la tupla "args"
		imprimir_argumentos_recursivo(std::make_index_sequence<sizeof...(Args)>{}, args);
	}
...
int main() {
	...
	MiPlantilla<int, float, char> objeto1(10,3.14, 'X');
	MiPlantilla<double, std::string> objeto2(4.6, "Hola");
	MiPlantilla<> objeto3; // sin argumentos de plantilla

	// Creamos un objeto "MiPlantilla" con tres argumentos de plantilla de diferentes tipos
	MiPlantilla<int, double, std::string> objeto1(10, 3.14, "Hola");

	// Llamamos a la función "imprimir_argumentos" del objeto "objeto1"
	objeto1.imprimir_argumentos();

	// Creamos otro objeto "MiPlantilla" con dos argumentos de plantilla de diferentes tipos
	MiPlantilla<float, int> objeto2(2.5, 7);

	// Llamamos a la función "imprimir_argumentos" del objeto "objeto2"
	objeto2.imprimir_argumentos();
